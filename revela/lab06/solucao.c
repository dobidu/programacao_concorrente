/* LAB-06 - SOLUÇÃO de referência: fila de blocos com backpressure.
 *
...
static int fila_tirar(Fila *f, Bloco *saida) {
    pthread_mutex_lock(&f->trava);
    while (f->quantos == 0 && !f->fim) {        /* LAÇO, não if */
        f->esperas_consumidor++;
        pthread_cond_wait(&f->nao_vazio, &f->trava);
    }
