/* LAB-12 - O mesmo servidor em uma thread, com epoll.   preparatório, sem nota
 *
 ...
 *   done
 *
 * ATENÇÃO - o resultado vai te surpreender, e a surpresa é o exercício.
 *
 * Medido nesta disciplina, com 128 clientes: thread-por-cliente ~740 ms de p50,
 * epoll ~1300 ms. O epoll perde, e perde feio. Isso não é bug no seu código.
 *
 * O reator resolve o problema de ter MUITAS CONEXÕES ABERTAS baratas - cada uma
 * custa uma struct em vez de uma pilha de 8 MB. Ele não resolve o problema de ter
 * MUITO TRABALHO DE CPU: aqui o servidor filtra 2,7 MB por requisição, e uma
 * thread só usa um núcleo dos 24 da máquina. O modelo com threads usa todos.
 *
 * "epoll é mais rápido" é uma dessas frases que se repete sem condição de
 * contorno. A frase certa é: o reator escala em número de conexões, não em
 * trabalho por conexão. Servidor de verdade combina os dois - um laço de eventos
 * para a I/O e um pool de workers para a CPU.
 *
 * O QUE VOCÊ TEM DE CONSEGUIR FAZER: o portão passando, a curva refeita ao lado da do LAB-11, e a explicação
 * de por que o epoll perde AQUI e em que forma de carga ele ganharia. Responder
 * "epoll é melhor" vale zero; a nota está em saber quando não é.
 */
#include "crono.h"
#include "filtro.h"
#include "revela.h"
    ...
    while (atendidos < s->quantos) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (c == NULL) {                        /* evento no socket de escuta */
                /* TODO: aceite TODAS as conexões pendentes, em laço, até accept
                 *       devolver erro. Uma por evento não basta: o epoll pode
                 *       reportar um único evento para várias chegadas.
                 *       Para cada uma: torne-a não-bloqueante, crie a Conexao e
                 *       registre-a no epoll com .data.ptr apontando para ela. */
                (void)conexao_nova; (void)nao_bloqueante;
                continue;
            }
