/* LAB-08 - SOLUÇÃO de referência: latch à mão, em C++17.
 *
...
public:
    ...
    void chegar_e_esperar() {
        ...
        if (--restantes_ == 0) {
            condicao_.notify_all();     /* o último abre para todos */
            return;
        }
        /* O predicado é obrigatório: cobre o despertar espúrio e o caso de a
         * thread chegar depois de o contador já ter zerado. */
        condicao_.wait(trava, [this] { return restantes_ == 0; });
    }
