/* LAB-04 - SOLUÇÃO de referência: false sharing, medido.
 *
 ...
 *
 *     make lab04 && ./lab04/solucao entrada.ppm saida.ppm --threads 8 --repeticoes 40
 */
#include "crono.h"
#include "filtro.h"
#include "revela.h"
...
/* Um acumulador por thread, cada um na sua linha de cache. */
typedef struct {
    unsigned long v;
    char _preenchimento[LINHA_CACHE - sizeof(unsigned long)];
} __attribute__((aligned(LINHA_CACHE))) AcumuladorAlinhado;
