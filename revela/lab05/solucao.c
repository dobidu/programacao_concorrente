/* LAB-05 - SOLUÇÃO de referência: três consertos para o mesmo contador, medidos.
 *
...
static void *trabalhar(void *arg) {
    ...
    for (int y = t->y0; y < t->y1; y++) {
        ...
        for (int x = 0; x < t->saida->largura; x++) {
            ...
            switch (t->modo) {
            case MODO_MUTEX:
                pthread_mutex_lock(t->trava);
                { unsigned long v = t->compartilhado[l];
                  JANELA();                       /* a janela continua aberta... */
                  t->compartilhado[l] = v + 1; }  /* ...mas agora ninguém entra */
                pthread_mutex_unlock(t->trava);
                break;
            case MODO_ATOMICO:
                __atomic_fetch_add(&t->compartilhado[l], 1UL, __ATOMIC_RELAXED);
                break;
            case MODO_REDUCAO:
                t->privado[l]++;                  /* só esta thread toca aqui */
                break;
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    if (modo == MODO_REDUCAO)               /* a redução: fora do caminho quente */
        for (int i = 0; i < o.threads; i++)
            for (int k = 0; k < 256; k++) baldes[k] += privados[(size_t)i * 256 + k];
