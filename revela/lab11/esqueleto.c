/* LAB-11 - Onde o thread-por-cliente joelha.   preparatório, sem nota
 *
 ...
 *   done
 *
 * O QUE VOCÊ TEM DE CONSEGUIR FAZER: a curva de p50 e p95 para 1, 2, 4, 8, 16, 32, 64 e 128 clientes, o
 * ponto onde ela deixa de ser plana, e uma frase sobre a causa. A vazão em img/s
 * ajuda: enquanto ela sobe, o servidor ainda está aproveitando os núcleos.
 */
#include "crono.h"
#include "filtro.h"
#include "revela.h"
...
static void *servir(void *arg) {
    ...
    for (int i = 0; i < s->quantos; i++) {
        ...
        if (c < 0) break;
        /* TODO: despache o atendimento desta conexão para uma thread própria,
         *       chamando atender() nela.
         *       Duas armadilhas: o descritor `c` muda a cada volta do laço, então
         *       passar &c para a thread é passar um alvo móvel; e ninguém vai
         *       chamar join, então a thread precisa ser detached ou vaza. */
        (void)c; (void)atender;
    }
    return NULL;
}
